...
def main():
    ...
    encrypted_file = input("Enter the encrypted file name (with .enc extension): ")
    ...
    if not encrypted_file.endswith('.enc'):
        print(f"Error: The file '{encrypted_file}' does not have the expected '.enc' extension.")
        return

    # Generate the output decrypted file name by removing the ".enc" extension
    output_file = encrypted_file[:-4]  # Remove ".enc" from the file name (5 characters)
